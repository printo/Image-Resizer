...
A powerful web-based application for batch resizing images based on CSV specifications. Upload a ZIP file containing images and a CSV file with resize dimensions, and get back perfectly resized images at 300 DPI.

## 🚀 Features

- **Batch Image Processing**: Resize multiple images simultaneously
- **CSV-Driven Specifications**: Define exact dimensions (length/width in inches) via CSV
- **High-Quality Output**: All images resized to 300 DPI for professional quality
- **Multiple Format Support**: Handles JPEG, PNG, WebP, and other common image formats
...
### Backend
**This is a 100% client-side application** - no backend server required! All processing happens in the user's browser using:
...
3. **Deploy static files**
   The `dist/` folder contains all static files ready for deployment to any web server (Nginx, Apache, Vercel, Netlify, etc.)
